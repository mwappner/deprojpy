...
height-map workflow. The image links below point to files generated by
`examples/02_plot_gallery.py`; run that script after cloning if the images do
not appear locally yet.
...
You can also point to explicit files:
...
The gallery script creates these files with one command:
